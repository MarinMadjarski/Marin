...
int main()
{
    fstream toolData("hardware.txt", ios::in | ios::out | ios::binary);
    if(!toolData)
    {
        cerr << "File could not be opened." << endl;
        toolData.close();
        ofstream toolCreate("hardware.txt", ios::binary);
        fillFile(toolCreate);
        toolCreate.close();
        toolData.open("hardware.txt", ios::in | ios::out | ios::binary);
    }
